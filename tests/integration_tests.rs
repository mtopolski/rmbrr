// Integration tests for win-rmdir-fast
...
fn delete_with_pipeline(path: &PathBuf) {
    ...
    let handles = worker::spawn_workers(worker_count, rx, broker);
    drop(tx);
...
fn test_large_tree_1000_files_100_dirs() {
    ...
    println!("Deleted {} dirs and {} files in {:?}", dir_count, file_count, elapsed);
...
fn test_wide_tree_1000_siblings() {
    ...
    assert_eq!(dir_count, 1001, "Should have 1001 dirs (root + 1000 siblings)");

    let start = Instant::now();
    delete_with_pipeline(&temp);
    let elapsed = start.elapsed();

    println!("Deleted wide tree ({} siblings) in {:?}", dir_count, elapsed);
    assert!(!temp.exists(), "Directory should be deleted");
}
...
fn test_directory_with_only_files() {
    ...
    println!("Deleted directory with {} files in {:?}", file_count, elapsed);
    assert!(!temp.exists(), "Directory should be deleted");
}
